...
class Trainer:
    def __init__(self, cfg: DictConfig) -> None:
        env_name = cfg.env.train.id
        execute_time = datetime.datetime.now().strftime("%m-%d-%Y_%H-%M")
        result_path = Path(cfg.tensorboard["result_dir"]) / '{}_{}'.format(env_name, execute_time)
        result_path.mkdir(exist_ok=True, parents=False)
        t_board= os.path.join(result_path, "tensorboard")
        ...
        self.ckpt_dir = result_path / Path('checkpoints')
        self.media_dir = result_path / Path('media')
        self.episode_dir = self.media_dir / 'episodes'
        self.reconstructions_dir = self.media_dir / 'reconstructions'

        self.writer = SummaryWriter(log_dir=t_board)
        ...
        for epoch in range(self.start_epoch, 1 + self.cfg.common.epochs):
            ...
            for metrics in to_log:
                for key, value in metrics.items():
                    if isinstance(value, np.ndarray): # make sure its a np array  before checking shape
                        if len(value.shape) == 5:
                            self.writer.add_video(tag=key, vid_tensor=torch.from_numpy(value), global_step=epoch, fps=5)
                    else:
    ...
    def eval_agent(self, epoch: int) -> None:
        ...
        if cfg_tokenizer.save_reconstructions:
            ...
            gif_data = make_reconstructions_from_batch(batch, save_dir=self.reconstructions_dir, epoch=epoch, tokenizer=self.agent.tokenizer)
            if epoch > cfg_tokenizer.start_after_epochs:
                metrics_tokenizer[f'{str(self.agent.tokenizer)}/eval/render'] = gif_data
